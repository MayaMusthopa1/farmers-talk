	...
	<table width="100%" border="3px" style="border-collapse:collapse;">
		...
		if($cari_informasi){
			if($inputan_pencarian != ""){
				$sql = mysql_query("select * from tb_informasi where judul like '%$inputan_pencarian%' or type like '%inputan_pencarian%'") or die (mysql_error());
			} else {
				$sql = mysql_query("select * from tb_informasi order by id desc") or die (mysql_error());
			}
		} else {
			$sql = mysql_query("select * from tb_informasi order by id desc") or die (mysql_error());			
		}
