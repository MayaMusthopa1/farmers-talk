<fieldset>
	...
	$sql = mysql_query("select * from tb_produk where kode_produk = '$kdproduk'") or die (mysql_error());
	...
	if($edit_produk){
		if($nama_produk=="" || $lokasi== "" || $nope_petani=="" || $harga=="" || $ketersediaan=="" || $nama_gambar=="" ){
			...
			if ($nama_gambar ==""){
					mysql_query("update tb_produk set nama_produk='$nama_produk', lokasi='$lokasi', harga='$harga', ketersediaan='$ketersediaan', gambar_produk='$nama_gambar' where kode_produk = '$kdproduk'") or die (mysql_error());
					?>
					<script type="text/javascript">
